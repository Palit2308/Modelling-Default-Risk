...
from Scripts.functions.merton_model.create_required_columns import create_reqd_columns_merton
...
def two_system_merton(df, k, E, sigma_E, D, rf, mu_a):

    df = create_reqd_columns_merton(df, k)

    df_merton = df[["cusip", "fyear", "mktval", "annualised_volatility", "debt_level", "rf", "equity_return", "default"]]

    df_merton.set_index(["cusip", "fyear"], inplace=True)

    df_merton['mu_a_max'] = df_merton[['rf', 'equity_return']].max(axis=1)
